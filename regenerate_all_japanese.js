...
async function regenerateAllJapaneseFiles() {
    console.log("🔄 Regenerating ALL Japanese files with content translation logic...\n");

    try {
        // Process kickstarter_all file
        console.log("📁 Processing kickstarter_all...");
        const englishAllPath = path.join(__dirname, 'results', 'kickstarter_all', 'kickstarter_english_all.json');
        const englishAllData = JSON.parse(await fs.readFile(englishAllPath, 'utf8'));
        
        console.log(`   Found ${englishAllData.results.length} projects in kickstarter_all`);
        
        // Apply enhanced translation logic
        const translatedAllResults = englishAllData.results.map((project, index) => {
            ...
            if (isEnglishPlatform) {
                ...
                if (japaneseProject['タイトル'] && typeof japaneseProject['タイトル'] === 'string') {
                    ...
                    if (index < 3) {
                        console.log(`     📝 [${index + 1}] Title: "${title}" → "${translatedTitle}"`);
                    }
                }
                
                // Enhanced description translation
                if (japaneseProject['説明'] && typeof japaneseProject['説明'] === 'string') {
                    const description = japaneseProject['説明'];
                    let translatedDesc = description
                        .replace(/\binnovative project\b/gi, '革新的なプロジェクト')
                        .replace(/\bexciting campaign\b/gi, 'エキサイティングなキャンペーン')
                        ...
                        .replace(/\bThis exciting campaign\b/gi, 'このエキサイティングなキャンペーン')
            ...
            if (japaneseProject['金額'] && typeof japaneseProject['金額'] === 'string') {
                japaneseProject['金額'] = '¥' + japaneseProject['金額'].replace(/[$]/g, '');
            }
            if (japaneseProject['支援金額'] && typeof japaneseProject['支援金額'] === 'string') {
                japaneseProject['支援金額'] = '¥' + japaneseProject['支援金額'].replace(/[$]/g, '');
            }
        ...
        const japaneseAllOutput = {
            ...
            強化率: englishAllData.results.length > 0 
                ? ((englishAllData.results.filter(r => r.ocr_enhanced).length / englishAllData.results.length) * 100).toFixed(2) + "%"
                : "0%",
            生成日時: new Date().toISOString(),
            処理サマリー: {
                総プロジェクト数: englishAllData.results.length,
                OCR強化済み: englishAllData.results.filter(r => r.ocr_enhanced).length,
                OCRエラー: englishAllData.results.filter(r => r.ocr_error).length,
                OCRなしで完了: englishAllData.results.filter(r => !r.ocr_enhanced && !r.ocr_error).length
            },
            ファイル: "kickstarter_japanese_all.json",
            言語: "japanese",
            フォルダ: "kickstarter_all",
            翻訳ノート: "すべてのデータは日本語に翻訳されています。英語プラットフォームのコンテンツは包括的なキーワードベース翻訳が適用されています。",
            結果: translatedAllResults
        };

        // Write enhanced Japanese file
        const japaneseAllPath = path.join(__dirname, 'results', 'kickstarter_all', 'kickstarter_japanese_all.json');
        await fs.writeFile(japaneseAllPath, JSON.stringify(japaneseAllOutput, null, 2), 'utf8');
        
        console.log(`✅ Enhanced Japanese file generated: ${japaneseAllPath}`);
        console.log(`📊 ${translatedAllResults.length} projects with comprehensive Japanese translation`);
